...
def simulate_volume(PDB, maxRes, mask_threshold, SSE_mask_threshold, SSE_type, minresidues):
    ...
    # Filter to maxRes
    if ok:
        ok = runJob("xmipp_transform_filter -i %s.vol -o %sFiltered.map --fourier low_pass %f 0.02 --sampling 1 -v 0"%(fnHash,fnHash,maxRes))
    # Create mask by thresholding
    if ok:
        ok = runJob("xmipp_transform_threshold -i %sFiltered.map -o %sMask.map --select below %f --substitute binarize -v 0"%(fnHash,fnHash,mask_threshold))
    # Obtain volumes
    if ok:
    ...
    # Create mask by thresholding
    if ok:
        ok = runJob("xmipp_transform_threshold -i %s_SSE.vol -o %sMask_SSE.map --select below %f --substitute binarize -v 0"%(fnHash,fnHash,SSE_mask_threshold))
    # Save mask
    if ok:
